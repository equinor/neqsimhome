...
2. **Knee point**: The knee point represents the best compromise — a point where further improvement in one objective requires a disproportionately large sacrifice in the other. This is often the most practical operating point.
...
for i, r in enumerate(debottleneck_results):
    ...
    gain_bbl_day = gain_kghr * 24.0 / (850.0 * 0.158987294928)  # illustrative liquid density 850 kg/m3
    revenue_musd_yr = gain_bbl_day * oil_price * operating_days / 1e6
    cost = upgrade_costs.get(r["bottleneck"], 50.0)  # MNOK
    exchange_rate_nok_per_usd = 10.0  # explicit scenario assumption
    payback = cost / (revenue_musd_yr * exchange_rate_nok_per_usd) if revenue_musd_yr > 0 else float('inf')
