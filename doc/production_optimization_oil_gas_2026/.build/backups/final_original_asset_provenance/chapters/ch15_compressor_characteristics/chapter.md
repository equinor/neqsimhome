...
![Figure 15.1: Specified speed-curve family illustrating centrifugal-compressor map terminology](figures/compressor_map_overview.png)

<!-- scientific-illustration:compressor_map_overview.png -->
The curves and vertical boundary markers are schematic. They are not vendor data or a qualified operating envelope; installed-map limits vary with speed and gas conditions.
...
At stonewall:
- Head and efficiency can fall sharply, but sonic choking does not require zero head
...
$$c^2=\left(\frac{\partial P}{\partial\rho}\right)_s,\qquad c\simeq\sqrt{\frac{\gamma ZRT}{MW}}\quad\text{(constant-property approximation)}.$$
...
![Figure 15.2: Illustrative pressure-flow bounds and two assumed operating points](figures/operating_envelope.png)

<!-- scientific-illustration:operating_envelope.png -->
The shaded region is a conceptual constraint window. No speed lines or constant-efficiency contours are shown, and the figure does not establish installed-machine limits.
<!-- /scientific-illustration -->
...
These transformations preserve a reduced-speed approximation and the flow coefficient. Exact Mach similarity additionally requires the actual speed of sound (including changing heat-capacity ratio and real-gas derivatives); Reynolds number, volume ratio and gas-property similarity must also be assessed.

### 15.4.3 Simplified Correction Factors

For corresponding points at the same flow coefficient and similar Mach/Reynolds conditions on the same machine,
...
where the coefficients $a_0, a_1, a_2, a_3$ define the curve shape. An illustrative normalized curve with negative slope at the design point is:

$$a_0=1.2,\quad a_1=0,\quad a_2=-0.2,\quad a_3=0$$
...
$$Q_{\text{recycle,min}} = \max\!\left[0,(1+\mathrm{SM}/100)Q_{\text{surge}}(N_{\text{max}})-Q_{\text{process,min}}\right]$$
...
Compression adds enthalpy to the circulating gas. An adiabatic recycle valve conserves enthalpy between its inlet and outlet; it does not add shaft work, and its temperature can decrease through the Joule–Thomson effect. Repeated compression without sufficient heat rejection raises the loop temperature. If the recycle gas is not cooled before returning to suction, the suction temperature increases progressively (thermal runaway), which:

- Increases the head required for a fixed pressure ratio and changes the map operating point
...
| Extended operation | Limited — thermal runaway risk | Sustained operation only within cooler, driver and control limits |
...
| Risk | Overheating time depends on gas inventory, power and heat rejection; calculate the transient | Cooler fouling, longer response |
...
|-------------|---------------------|
| MW decrease | Usually increases required J/kg for a specified pressure ratio; recalculate the map point |
| $\gamma$ change | Changes temperature rise, sonic speed and efficiency conversion |
| $Z$ change | Changes density and real-gas compression path |
| $T$ increase | Increases inlet volume at fixed mass flow and typically increases required head |
...
Plot the corrected operating point at a declared actual or referred coordinate basis. Composition changes alter reduced speed, density and required pressure-ratio head; the entire actual head-flow map does not universally shift up/right for lighter gas.
...
- Transient risk if one machine trips: header inventory, check-valve closure and controls determine the response. Increased forward flow with lower required head can move surviving machines toward high-flow choke; surge cannot be inferred from that condition alone
...
API 617 (9th Edition, 2022) specifies the requirements for centrifugal compressor testing. The factory acceptance test (FAT) verifies that the compressor meets its guaranteed performance at the specified conditions.
...
Performance acceptance must use the purchase specification, applicable API617 edition, specified ASME PTC10/ISO5389 test method, guarantee point and measurement uncertainty. Do not interpret a generic head/efficiency/power percentage table as an API acceptance clause. Record the contract tolerances and compare corrected test results on the same reference basis.
...
$$H_{\mathrm{compressor}}(Q)=H_{\mathrm{required}}(P_s,P_d,T_s,\mathbf{z},Q)$$
...
![Figure 15.3: Complete compressor performance map with head and efficiency curves at multiple speeds](figures/compressor_map_complete.png)

Generated compressor performance map showing (top) polytropic head vs. actual inlet volume flow at five speeds from 70% to 105% of design, with the surge line marked in red; and (bottom) polytropic efficiency vs. flow at the same speeds. The design point is at 100% speed, 4500 m$^3$/hr, with 80% polytropic efficiency.
...
![Figure 15.4: Compressor Head vs. Flow — Multi-Speed Map. Assumed multi-speed teaching map](figures/ch13_head_vs_flow.png)
...
![Figure 15.5: Compressor Efficiency vs. Flow. Assumed multi-speed teaching map](figures/ch13_efficiency_vs_flow.png)
...
![Figure 15.6: Compressor Operating Envelope. Assumed teaching surge and control boundaries](figures/ch13_operating_envelope.png)
...
**Screw compressors** (twin-screw or single-screw) use meshing helical rotors to compress gas in a continuous positive-displacement process. They are used for low-pressure boosting (< 10 bara discharge), wet gas compression, and applications where liquid tolerance is required. Key characteristics: may tolerate a specified liquid fraction in qualified oil-flooded designs; slug size, liquid composition and thermal limits require vendor approval, continuous flow with low pulsation, compact and robust. Limitations: limited to low-to-moderate pressure ratios (< 5:1), lower efficiency than centrifugal at high flows, and internal leakage limits performance at high pressure ratios.
...
2. **If required discharge pressure > 250 bara OR pressure ratio > 6:1 per casing**: Evaluate reciprocating and purpose-designed high-pressure centrifugal machines; pressure alone does not mandate one type. This includes gas reinjection compressors (300–500 bara), wellhead gas compression with high GOR, and small gas-to-wire applications.
...
1. API Standard 617 (2022). Axial and Centrifugal Compressors and Expander-Compressors, 9th ed. American Petroleum Institute.
...
13. NORSOK P-002:2023+AC:2024 (2023). Process System Design. Standards Norway.
