...
- **Reverse acting**: When the measurement increases, the output decreases. Example: level controller on a liquid outlet valve — when level rises, open the valve more (OP increases), but in NeqSim the convention is that `setReverseActing(true)` means the controller output increases when the error is positive (PV < SP).
...
PT100 = PressureTransmitter("PT-100", sep.getGasOutStream())
PT100.setUnit("bara")
...
TT100 = TemperatureTransmitter("TT-100", sep.getGasOutStream())
TT100.setUnit("C")
process.add(TT100)
...
LC100.setControllerSetPoint(48.0)            # Target level = 1.2 m
...
# --- Run steady state ---
process.run()
print(f"Initial level: {sep.getLiquidLevel() * sep.getInternalDiameter():.2f} m")
...
for i in range(n_steps):
    ...
    level_arr[i] = sep.getLiquidLevel() * sep.getInternalDiameter()
...
![Dynamic response of separator level, pressure, and temperature to a feed rate disturbance](figures/ch20_separator_dynamic_response.png)
...
2. **Pressure response**: The increased gas flow causes a temporary pressure rise, which the pressure controller handles by opening the gas valve.

3. **Return to steady state**: After the disturbance is removed at $t = 20$ min, both controllers bring the process back to the set point. The level controller overshoot and settling time depend on the tuning parameters.
...
# Add transmitters
PT_vessel = PressureTransmitter("PT-vessel", vessel.getGasOutStream())
PT_vessel.setUnit("bara")
process.add(PT_vessel)

TT_vessel = TemperatureTransmitter("TT-vessel", vessel.getGasOutStream())
TT_vessel.setUnit("C")
...
ax1.axhline(y=7.91325, color='r', linestyle='--', label='Illustrative target (6.9 barg)')
...
![Pressure and temperature profiles during vessel blowdown](figures/ch20_blowdown_curves.png)
...
LC.setControllerSetPoint(33.3333333333)
...
for i in range(n_steps):
    ...
    process.runTransient(dt)
    level_s[i] = sep.getLiquidLevel() * sep.getInternalDiameter()
...
![Separator level response during slug arrival with averaging level control](figures/ch20_slug_response.png)
...
6. **Run dynamic simulation** with `process.runTransient(dt)` in a loop. Choose a timestep that is:
...
![Figure 29.1: Level Controller Response](figures/ch20_level_controller_response.png)

*Figure 29.1: Level Controller Response*

![Figure 29.2: Pid Tuning Comparison](figures/ch20_pid_tuning_comparison.png)
