...
- **`isHardLimitExceeded()`** returns `true` if any HARD-type constraint exceeds its maximum value. This indicates a safety or trip condition.
...
List<String> above80 = process.getEquipmentNearCapacityLimit();
```
...
List<String> tier1 = process.getEquipmentNearCapacityLimit();  // Critical
List<String> tier2 = process.getEquipmentNearCapacityLimit();  // Warning
List<String> tier3 = process.getEquipmentNearCapacityLimit();  // Watch
...
7. **Economic Evaluation**: For the platform in Exercise 6, estimate the production gain from debottlenecking each of the top three constraints. If oil price is $70/bbl and the modifications cost 20, 45, and 80 MNOK respectively, calculate the payback period for each option.
...
![Figure 21.1: Debottleneck Cascade](figures/ch21_debottleneck_cascade.png)
