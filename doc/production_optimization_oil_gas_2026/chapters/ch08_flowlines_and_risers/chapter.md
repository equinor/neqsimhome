...
![Figure 8.1: Conceptual sequence of wellhead, flowline, riser, receiving and separation functions](figures/pipeline_system_overview.png)

<!-- scientific-illustration:pipeline_system_overview.png -->
The printed dimensions and inlet pressure are schematic inputs. This block diagram is not a cross-section or an accepted hydraulic sizing result.
...
# NeqSim Sm3 uses 15 C and 1.01325 bara. Convert through the EOS, including Z.
reference = gas.clone()
...
print(f"Inlet-property screening drop: {dP_bar:.1f} bar over {L/1000:.0f} km")
...
| Slug → Annular | High Froude number ($Fr_G > 1.5$) | $v_{SG} > 3.1 \left[\frac{\sigma g (\rho_L - \rho_G)}{\rho_G^2}\right]^{1/4}$ |
...
$$
-\frac{dP}{dL} = \frac{f_{tp}\rho_n v_m^2/(2D)+\rho_s g\sin\theta}{1-\rho_s v_m v_{SG}/P}
$$
...
**Step 3: Correct for inclination** using the Beggs–Brill inclination multiplier. A separately enabled Payne correction is an additional empirical holdup adjustment; it is not the definition of this multiplier:
...
Mechanistic models can represent additional mechanisms and transitions, but extrapolation beyond their closure-correlation validation range is not automatically more accurate.
...
NeqSim divides the pipeline into increments (set by `setNumberOfIncrements`) and updates pressure, temperature and local phase properties along the route. The following diameter comparison reports only each line's outlet pressure and temperature. These endpoint results must not be read as a plotted profile; the stored increment data can be inspected separately.
...
For a riser in deepwater, the hydrostatic head of the liquid holdup creates a substantial pressure decrease from the riser base to the topside in upward flow. For example, with $H_L = 0.3$ and typical fluid densities ($\rho_L = 700$ kg/m³, $\rho_G = 80$ kg/m³):
...
With inclination positive upward, gravity dominates friction in a descending, full-bore liquid segment when
...
**Pigging in hilly terrain:** Pigs traveling through undulating terrain accumulate liquid ahead of them in each downhill section. This creates progressively larger liquid slugs as the pig advances. An inventory screening bound for liquid collected from the listed sections is:
...
| Pressure drop | 1–3 bar/10 km (transport) | 0.5–2 bar/10 km | 2–5 bar/10 km |
...
Determine the stabilizing gas-lift range with the selected transient/stability model and check the extra friction and compression duty. Confirm the operating range against measured riser behavior; no universal static-pressure ratio guarantees stability.
...
![Figure 8.2: Illustrative seabed and riser elevation profile relative to sea level](figures/flowline_riser_profile.png)

<!-- scientific-illustration:flowline_riser_profile.png -->
The negative vertical coordinate denotes elevation below sea level. This is a geometric sketch, not a pressure/temperature profile; it must be supplied to a hydraulic and thermal calculation.
<!-- /scientific-illustration -->
...
![Figure 8.3: Pressure Drop vs Pipeline Diameter (Flow = 50 t/hr, L = 10 km, horizontal)](figures/fig01_pressure_drop_vs_diameter.png)
...
![Figure 8.4: Pressure Drop vs Flow Rate (Diameter = 8 inch, L = 10 km, horizontal)](figures/fig02_pressure_drop_vs_flow.png)
...
![Figure 8.5: Liquid Holdup vs Gas Velocity (8-inch pipe, 5 km, horizontal)](figures/fig03_liquid_holdup.png)
...
![Figure 8.6: Pipeline Capacity Curve (Available ΔP = 20 bar, L = 10 km)](figures/fig04_pipeline_capacity.png)
...
5. API RP 14E (1991). *Recommended Practice for Design and Installation of Offshore Production Platform Piping Systems*. American Petroleum Institute.
