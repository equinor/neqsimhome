...
gas.init(0)  # Initialize mole fractions before ISO 6976 reads them.
...
![Figure 19.1: Calculated GCV and superior Wobbe index versus C₂+ content on the declared ISO6976 basis](figures/gcv_wobbe_vs_ngl.png)
...
Below freezing, the worked water calculation is an aqueous VLE limit: stable ice and hydrate equilibria are excluded. Both sides of each reported dew point are checked by fresh TP flashes, with aqueous liquid present 0.2 K below and absent 0.2 K above. This distinction matters when a contract specifies a frost point or hydrate margin.
...
# The legacy continuation may end with paired undefined terminal slots.
# Retain finite contiguous data; never bridge an undefined internal gap.
import numpy as np
raw_dew_T = np.asarray(dew_temps, dtype=float)
raw_dew_P = np.asarray(dew_press, dtype=float)
valid_dew = np.isfinite(raw_dew_T) & np.isfinite(raw_dew_P) & (raw_dew_T > 0) & (raw_dew_P > 0)
discarded_padding_slots = int(np.sum(~valid_dew))
assert np.all(valid_dew[:int(np.sum(valid_dew))])
assert np.all(~valid_dew[int(np.sum(valid_dew)):])
print("Undefined terminal envelope slots excluded:", discarded_padding_slots)
dew_temps = raw_dew_T[valid_dew]
dew_press = raw_dew_P[valid_dew]
assert len(dew_temps) > 20
...
print(f"Sampled dew-curve maximum: {max_T_C:.1f} °C at {cricondentherm_P:.1f} bara")
...
![Figure 19.2: NeqSim SRK vapour-liquid saturation envelope for the explicitly specified fluid in Section 19.6.2. The traced maxima are 27.3 °C (cricondentherm) and 111.0 bara (cricondenbar). Markers are sampled continuation extrema; no critical point or production/export trajectory is inferred. Dew/bubble assignment was checked by fresh TP flashes at three pressures around each branch. Continuation gaps remain open; solids, hydrates and aqueous stability are outside this VLE calculation.](figures/phase_envelope_export_gas.png)

<!-- scientific-illustration:phase_envelope_export_gas.png -->
Input relative molar amounts are nitrogen 0.008, CO2 0.015, methane 0.88, ethane 0.055, propane 0.02, i-butane 0.005, n-butane 0.007, i-pentane 0.003, n-pentane 0.002, n-hexane 0.003, n-heptane 0.002, normalized to mole fractions. Fresh TP flashes bracketed each saturation branch at three sampled pressures; phase amounts and density continuity determined physical branch assignment. Component closure and fugacity equality verify these computed states, but do not establish agreement with measured mixture saturation data. 
...
for d_inch in diameters_inch:
    ...
    feasible = math.isfinite(P_out) and 0.0 < P_out <= 150.0
...
![Figure 19.3: Accepted 300 km pipeline pressure-drop and inlet-velocity results at 15 MSm³/day](figures/pipeline_sizing_dp_vs_diameter.png)
...
![Figure 19.4: Gas Export Pipeline: Pressure Profile](figures/ch17_pipeline_profiles.png)
...
![Figure 19.5: Pressure Drop vs Pipeline Diameter (150 km, 5 MSm3/day)](figures/ch17_diameter_sensitivity.png)
...
- **NeqSim modeling** — ISO 6976 gas quality via `Standard_ISO6976_2016`, water VLE dew point via `waterDewPointTemperatureMultiphaseFlash()` with onset-bracketing checks, HC dew point via phase envelope (`calcPTphaseEnvelope`), and pipeline hydraulics via `PipeBeggsAndBrills`
