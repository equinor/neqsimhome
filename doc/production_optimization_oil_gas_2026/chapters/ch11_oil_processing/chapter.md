...
Reid vapor pressure is a test-method quantity at 37.8 °C and is an absolute pressure under ASTM D323; a thermodynamic bubble-pressure calculation for the specified liquid is TVP and must not be reported as a D323 test result.\cite{foundationASTMD3232026} to Oil Processing
...
![Figure 11.1: Schematic of a typical offshore oil processing train showing multi-stage separation, dewatering, and stabilization](figures/oil_processing_train_overview.png)
...
$$\text{CaCl}_2 + 2\text{H}_2\text{O} \rightarrow \text{Ca(OH)}_2 + 2\text{HCl}$$
...
**Reboiler duty** determines vapor traffic, but bottoms flow is not itself a vaporization rate. For the complete steady column, define positive $Q_R$ into the reboiler and positive $Q_C$ removed in the condenser:

$$Q_R-Q_C=\dot H_D+\dot H_B-\dot H_F.$$

All enthalpy flows are in W on one reference basis. An unrefluxed column has $Q_C=0$; any feed heater is outside this boundary. For the reboiler alone, use its incoming liquid, outgoing vapor and bottoms enthalpies. The checked examples specify heat input directly and verify the individual-stage and complete-column balance.
...
Reid vapor pressure is defined by the ASTM D323 apparatus and conditioning procedure at 37.8°C. ASTM D5191 is a separate mini-method with a specified conversion/reporting basis, not simply automated D323. A bubble-pressure flash predicts equilibrium true vapor pressure at its specified temperature; it is not a Reid test. Finite vapor/liquid ratio, air saturation and sample preparation prevent a universal conversion from a bubble-point calculation to measured RVP.
...
where $x_i$ is the volume fraction and $\mu_i$ is the viscosity of each component. This simple logarithmic mixing rule is approximate; ASTM D341/Walther fits viscosity versus temperature for a given petroleum liquid, not arbitrary blend viscosity. Use measured blend data or a validated blending correlation at the same temperature; logarithmic mixing is only a screening estimate.
...
plt.ylabel("Final 1.5 bara Liquid Rate (kg/hr)", fontsize=12)
...
tvp_37_8 = calculate_tvp(stream, 37.8)
tvp_50 = calculate_tvp(stream, 50.0)
tvp_60 = calculate_tvp(stream, 60.0)

print(f"TVP (at 37.8°C): {tvp_37_8:.1f} kPa")
...
The first column has five equilibrium contacting stages plus an equilibrium reboiler, no condenser and a specified60 kW heat input. Top/bottom pressures are8/8.5 bara; the calculated bottoms temperature is about 107.1°C and both products have positive flow. The second, deliberately smaller comparison uses one contact plus reboiler and 400 kW on a different feed condition. Strict MESH and individual-stage balances are checked for both. Comparing their oil yields with a flash does not establish superior recovery at equal product quality: pressures, feed heating, duty and vapor-pressure specifications must first be equalized.
...
# Unrefluxed stripping: specify heat input in the stage energy equations
stabilizer.getReboiler().setHeatInput(60000)
stabilizer.setTopPressure(8.0)
stabilizer.setBottomPressure(8.5)
stabilizer.setSolverType(jneqsim.process.equipment.distillation.DistillationColumn.SolverType.DIRECT_SUBSTITUTION)
stabilizer.setMaxNumberOfIterations(100, True)
stabilizer.setTemperatureTolerance(1e-7)
stabilizer.setMassBalanceTolerance(1e-6)
stabilizer.setEnthalpyBalanceTolerance(1e-5)
stabilizer.setEnforceEnergyBalanceTolerance(True)
stabilizer.setMeshResidualTolerance(1e-5)
stabilizer.setEnforceMeshResidualTolerance(True)
...
assert str(stabilizer.getLastSolveStatus()) == "RIGOROUS_CONVERGED"
assert stabilizer.getLastMeshResidualNorm() < 1e-5
assert stabilizer.getGasOutStream().getFlowRate("kg/hr") > 0.0
assert stabilizer.getReboiler().getLiquidOutStream().getFlowRate("kg/hr") > 0.0
...
# --- One contacting stage plus equilibrium reboiler, 400 kW ---
...
stabilizer = jneqsim.process.equipment.distillation.DistillationColumn(
    "Stabilizer", 1, True, False)
stabilizer.addFeedStream(col_feed, 1)
stabilizer.getReboiler().setHeatInput(400000)
stabilizer.setTopPressure(8.0)
stabilizer.setBottomPressure(8.5)
stabilizer.setSolverType(jneqsim.process.equipment.distillation.DistillationColumn.SolverType.DIRECT_SUBSTITUTION)
stabilizer.setMaxNumberOfIterations(100, True)
...
# Different feed pressure, temperature and duty: this is NOT an equal-quality comparison.
print(stabilizer.getConvergenceDiagnostics())
assert str(stabilizer.getLastSolveStatus()) == "RIGOROUS_CONVERGED"
assert stabilizer.getLastMeshResidualNorm() < 1e-5
assert stabilizer.getGasOutStream().getFlowRate("kg/hr") > 0.0
...
print(f"Liquid-yield difference:  {col_oil_rate - flash_oil_rate:.1f} kg/hr "
...
| Coriolis | Mass flow via tube vibration | ±0.10% | Direct mass measurement; calibration/proving requirements still apply | High cost, size limited (< 10") |
...
A prover loop is a precisely calibrated pipe section used to determine the meter calibration. A K-factor is pulses per unit volume; a meter factor is a dimensionless correction to indicated quantity. The standard types are:
...
With both volumes corrected to one common basis, distinguish

$$K=\frac{N_{pulses}}{V_{reference}},\qquad MF=\frac{V_{reference}}{V_{indicated}}.$$

The first has units pulses/m³ and the second is dimensionless. A prover calculation includes calibrated base volume, steel temperature/pressure corrections and fluid temperature/pressure corrections as applicable; multiplying only the base volume by fluid CTL/CPL is not a complete proving procedure.
...
![Figure 11.3: True vapor pressure vs stabilizer temperature](figures/fig10_1_rvp_vs_temperature.png)
...
![Figure 11.4: Oil API Gravity and Density vs Separator Pressure](figures/fig10_2_api_vs_pressure.png)
...
![Figure 11.5: Oil Quality vs Number of Separation Stages](figures/fig10_3_multistage_oil_quality.png)
...
![Figure 11.6: Gas Shrinkage — Gas Released at Each Pressure](figures/fig10_4_gas_shrinkage.png)
...
The accepted reboiled columns use an explicit heat input and satisfy the strict numerical column status, MESH residual, stage/boundary mass and energy checks, and phase-fugacity checks. They illustrate the selected stripping conditions, not an unmeasured product specification. Bubble-pressure calculations are labeled TVP; a Reid vapor-pressure claim requires the stated test-method basis.\cite{foundationASTMD3232026}

The calculation and literal-code records are in `verification/scientific_revision/ch11_manuscript_physics.json`; the chapter scope and code hashes are indexed in `foundations_review.json`.
...
**Exercise 11.8**: A field produces 28° API crude with a viscosity of 85 mPa·s at 40°C and a pour point of 24°C. Estimate blend viscosity with an explicitly assumed mixing rule and identify the additional wax/blend test needed for pour point when the crude is diluted with 30vol%condensate (55° API, 0.3 mPa·s viscosity).
