...
Produced water is the largest waste stream by volume in oil and gas production. Produced-water volumes depend strongly on field age, reservoir connectivity and injection strategy; global water-to-oil ratios and annual NCS totals require a dated inventory and are not design inputs for an individual facility. As fields mature, the water cut increases — often reaching 90–95% in late-life operations — making produced water management one of the most significant technical and economic challenges in production optimization.
...
Dissolved hydrocarbons include BTEX (benzene, toluene, ethylbenzene, xylenes), naphthalene, phenols, and polycyclic aromatic hydrocarbons (PAHs). Unlike dispersed oil, dissolved hydrocarbons are not removed efficiently by gravity separation or conventional deoiling hydrocyclones; adsorption, stripping and suitable membranes are different physical separation mechanisms and require different treatment approaches.
...
The OSPAR reference is Agreement 2005-15, based on a modified ISO 9377-2 extraction/GC-FID procedure. The 30 mg/L performance standard is a flow-weighted monthly average; site permits can impose additional sample, alarm or discharge-stop conditions.\cite{foundationOSPARmethod} The averaging calculation is:
...
The separation efficiency of a hydrocyclone can be described by a grade efficiency curve, which gives the probability of separation as a function of droplet size. A cut-size correlation must be calibrated for the actual liner geometry and droplet population. A dimensionally consistent relative screening relation for a fixed liner is
...
- **Pressure drop ratio (PDR)**: $\mathrm{PDR}=(P_{\mathrm{feed}}-P_{\mathrm{reject}})/(P_{\mathrm{feed}}-P_{\mathrm{clean}})$. Typical PDR range is 1.5–3.0; higher PDR improves separation but increases energy consumption
...
|--------|-----------|-------------|---------------|-----------|
| Solvent extraction and gas chromatography (modified ISO 9377-2) | GC-FID after extraction | 0.1–1,000 | 30–60 min (lab) | Regulatory reference |
...
A positive saturation index indicates thermodynamic supersaturation. Precipitation rate and deposition location additionally depend on nucleation, residence time, inhibitors and transport; SI > 0 alone does not establish plugging. The saturation index for a mineral $AB$ is:

$$
SI = \log_{10}\left(\frac{[\text{A}^{m+}][\text{B}^{n-}]}{K_{sp}(T, P)}\right)
$$

where activities are dimensionless relative to the selected standard state and $K_{sp}$ uses that same convention. The displayed expression is for a 1:1 mineral; general minerals require stoichiometric activity powers. Concentration products without activity coefficients are only a dilute approximation.
...
**Barium sulfate** is the most problematic offshore scale because it is extremely insoluble and is poorly soluble in conventional mineral-acid treatments; qualified chelant formulations can remove some deposits. Prevention (scale inhibitor injection) is the primary strategy.

Scale potential depends on the actual ion inventory, not a universal seawater fraction. With additive water volumes, ion concentrations $C$ in kg/m³ and molecular weights $M$ in kg/mol, a complete-precipitation upper bound is
...
When formation water rich in barium ions mixes with injection seawater rich in sulfate ions, barium sulfate supersaturation can develop; precipitation must be assessed for the mixed composition and operating path. A rigorous scale risk assessment involves:
...
Use the predicted supersaturation and upper-bound precipitate mass to select laboratory compatibility and dynamic deposition tests. There is no universal mg/L threshold below which inhibitor is unnecessary. Compare continuous dosing, squeeze treatment and sulfate-removal options using measured inhibitor performance, residual sulfate, formation-water chemistry and injection history; sulfate removal reduces risk but cannot guarantee zero scale.
...
$$
P = \frac{RT}{V_m - b} - \frac{a(T)}{V_m(V_m + b)} - \frac{RT}{2V_m} \left[1+\frac{1}{V_m}\frac{\partial \ln g}{\partial (1/V_m)}\right] \sum_i x_i \sum_{A_i}(1 - X_{A_i})
$$
...
This nonlinear rational dependence on water cut drives the economic justification for water treatment upgrades, water shut-off treatments, and infill drilling strategies that target low-water-cut zones.
...
![Figure 13.1: Specified oil/water production scenario with water cut calculated from the same liquid volumes](figures/water_lifecycle_profile.png)

<!-- scientific-illustration:water_lifecycle_profile.png -->
...
![Figure 13.2: Oil vs Water Production Rate](figures/ch16_watercut_sensitivity.png)
...
![Figure 13.3: Separator Pressure Effect on Phase Split (40 mol% water)](figures/ch16_pressure_effect.png)
...
The process-unit examples are checked for material/enthalpy consistency. Salt mass-fraction conversion and the limiting-reactant barite inventory are independently checked; supersaturation and an inventory upper bound do not predict deposited scale or treatment kinetics. The OSPAR statement is tied to the flow-weighted monthly performance standard and its reference analytical method, with site-permit conditions separate.\cite{foundationOSPAR2023,foundationOSPARmethod}

The calculation and literal-code records are in `verification/scientific_revision/ch13_manuscript_physics.json`; the chapter scope and code hashes are indexed in `foundations_review.json`.
...
A gravity separation vessel operates at 60 °C with oil density 830 kg/m³ and water density 1,020 kg/m³. (a) Using Stokes' law, calculate the terminal settling velocity for oil droplets of diameter 10, 20, 50, 100, and 200 µm. Use water viscosity of 0.47 cP. Calculate droplet Reynolds number for each result; replace Stokes drag with an appropriate finite-Re drag relation where creeping flow is not satisfied. (b) If the vessel has an effective separation area of 25 m² and a water throughput of 5,000 m³/day, what is the minimum droplet size that will be separated? (c) How does the answer change if the temperature drops to 30 °C (water viscosity 0.80 cP)?
...
1. Fakhru'l-Razi, A., Pendashteh, A., Abdullah, L. C., Biak, D. R. A., Madaeni, S. S., and Abidin, Z. Z. (2009). "Review of technologies for oil and gas produced water treatment." *Journal of Hazardous Materials*, 170(2-3), 530–551.
