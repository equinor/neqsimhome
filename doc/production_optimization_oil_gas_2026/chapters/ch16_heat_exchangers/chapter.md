...
| TEMA Class | Service emphasis | Pressure basis |
|-----------|------------------|----------------|
| R | Petroleum and heavy-duty | Actual design pressure and vessel code |
| C | General commercial | Actual design pressure and vessel code |
| B | Chemical service | Actual design pressure and vessel code |

Most oil and gas exchangers are designed to TEMA R standards.

![Figure 16.1: Schematic circular shell cross-section with an illustrative tube arrangement](figures/shell_tube_hx_cross_section.png)

<!-- scientific-illustration:shell_tube_hx_cross_section.png -->
The drawing identifies a shell, tubes and baffle concepts. It does not specify a complete TEMA construction, dimensions, tube count for a design, or a calculated heat-transfer area.
...
PCHEs serve selected compact high-pressure duties, including CO₂ applications. A diffusion-bonded PCHE is distinct from a brazed-aluminum plate-fin exchanger commonly used in cryogenic cold boxes; do not transfer their pressure, fouling or material limits between technologies. Their main limitation is that they cannot be mechanically cleaned, so they require clean fluids.
...
Multiplying by the outside area gives the following equivalent cylindrical-wall expression (no thin-wall approximation is required):
...
Here $R=C_c/C_h$ and $P$ is the cold-side temperature-rise fraction, not necessarily the exchanger effectiveness based on $C_{min}$. Use the analytic limit or a numerically stable evaluation near $R=1$. The correction factor for a 1-2 exchanger (one shell pass, two tube passes) is:
...
For $C_r=1$, the counterflow limit is $\varepsilon=\mathrm{NTU}/(1+\mathrm{NTU})$. For $C_r\to0$, it is $1-e^{-\mathrm{NTU}}$. These relations assume constant heat-capacity rates; for mixture phase change use enthalpy segments and check internal temperature approaches.
...
A temperature cross alone does not prove that a single shell is infeasible. Evaluate the appropriate effectiveness or F-factor relation for the actual pass arrangement and both terminal approaches. Add shells, change configuration or revise targets when the calculated thermal feasibility or chosen F-factor criterion fails.
...
![Figure 16.2: Specified noncrossing hot and cold composite curves with a 25 K minimum vertical temperature gap](figures/composite_curves.png)

<!-- scientific-illustration:composite_curves.png -->
...
The vertical temperature separation at a common enthalpy coordinate represents the local temperature driving force; the graphical area between curves is not itself that temperature difference. The pinch point is where the curves are closest (separated by $\Delta T_{\min}$).
...
- **Environmental regulations** — discharge temperature, mixing-zone and intake/outfall limits are site-permit inputs; there is no universal 7 °C allowance
- **Intake depth** — deeper intake provides colder, cleaner water

### 16.12.2 Material Selection

| Material family | Required service qualification |
|-----------------|--------------------------------|
| Titanium | Chloride service, galvanic pairing, crevices and the actual temperature/pressure envelope |
| Copper–nickel | Velocity, impingement, sulfide contamination and protective-film conditions |
| Super duplex | Pitting/crevice corrosion, chlorination, temperature, weld condition and stress; a 250 °C generic seawater rating is not valid |
| Fiber-reinforced polymer | Resin system, pressure, temperature, fire exposure and installation standard |

Select material using the actual seawater chemistry, temperature, chlorination and design code with qualified vendor data; thermal stability alone is not corrosion suitability.
...
$$
W_{\text{stage}} = \frac{1}{\eta_p}\frac{n}{n-1}\dot{m}\frac{RT_{\text{inlet}}}{M} \left[\left(\frac{P_{\text{out}}}{P_{\text{in}}}\right)^{(n-1)/n} - 1\right]
$$
...
Gas export specifications define water and hydrocarbon dew points at a stated pressure or over a stated pressure interval. Cricondentherm is the maximum saturation temperature on the mixture envelope, not a universal standard. Cooling followed by condensate removal changes gas composition and can improve hydrocarbon dew point; a cooler alone does not remove the condensed material or establish compliance. Check the final export composition against the contract pressure basis.
...
1. **Revise the achievable outlet target** — insufficient heating gives a colder outlet; insufficient cooling gives a warmer outlet. Accept a change only if downstream constraints remain satisfied
...
![Figure 16.3: Counterflow temperatures on a heat-transfer coordinate. Linear interpolation between NeqSim endpoint temperatures on a heat-transfer coordinate](figures/ch14_temperature_profile.png)
...
![Figure 16.4: Heat Exchanger Sensitivity — Duty & Temperatures vs. Flow](figures/ch14_duty_vs_flow.png)
...
![Figure 16.5: Recovered heat duty and thermal effectiveness versus overall conductance UA for the specified two-stream heat exchanger](figures/ch14_ua_sizing.png)
...
10. GPSA Engineering Data Book (2016). 14th Edition, Gas Processors Suppliers Association.
