...
A process simulator is commonly used to solve a fixed problem: given a feed composition, flow rate, and equipment configuration, calculate the outlet conditions. But production optimization asks a fundamentally different question — *what is the best operating point?* Answering this requires varying decision variables (flow rates, pressures, temperatures), re-solving the simulation at each trial point, and checking whether all equipment constraints are satisfied.

General-purpose optimization libraries (SciPy, MATLAB, GAMS) can perform the search, but they know nothing about separators, compressors, or surge margins. A simulator may also include optimization tools, but the search still needs explicit decisions, objectives and acceptance criteria. The gap between the two is bridged by an **optimization framework** — a software layer that:
...
![Figure 23.1: Conceptual layering of interfaces, optimization, process simulation and thermodynamic models](figures/fig25_1_architecture.png)

<!-- scientific-illustration:fig25_1_architecture.png -->
The drawing groups software responsibilities. Class-level interfaces and accepted evidence checks are specified in the text; this schematic is not an exhaustive dependency or deployment diagram.
...
- **Constraint engine** — the `CapacityConstrainedEquipment` interface that exposes configured limits for participating equipment
- **Optimizer** — the `ProductionOptimizer` and `ProcessOptimizationEngine` classes that explore the decision-variable space, query the simulator, and evaluate the constraints explicitly included in the model

This chapter explains how each layer works and how they combine into a coherent optimization framework. Chapter 22 introduces optimization theory; Chapter 24 develops the production workflow. Here, we focus on the software architecture — the classes, interfaces, and APIs that make it all work — so that the reader can extend the framework for site-specific problems.
...
The process topology determines the **coupling structure** of the optimization problem. In a linear topology (feed → separator → compressor → cooler → export), changing the feed rate affects downstream equipment, but monotonic feasibility must be checked across the chosen bounds. The optimizer can use efficient single-variable search methods such as binary search or golden-section search.
...
A practical consequence is that the engineer should consider the process topology when selecting a search algorithm. Use binary feasibility only after establishing a monotonic feasible interval, and golden-section only for a unimodal score; linear topology alone establishes neither property. Topologies with recycles or parallel trains often require `NELDER_MEAD_SCORE` or `PARTICLE_SWARM_SCORE` for robustness. Table 23.2 in Section 23.5 provides detailed guidance.
...
|------|---------|---------|----------------------|
| **HARD** | Absolute physical or mechanical limit that cannot be exceeded | Compressor maximum speed, valve fully open, separator MAWP | Configured acceptance limit; violation is infeasible, but this flag does not simulate a trip |
| **SOFT** | Design-basis limit that may be temporarily exceeded with degraded performance | Design flow rate, recommended retention time | May be penalized; no automatic physical efficiency reduction follows from the label |
...
Defaults and sizing calculations are equipment-specific. Discover the resulting dimensions, chart state, design values and enabled constraints after sizing. Existing equipment requires its actual ratings; an inferred design point is not nameplate evidence.
...
These convenience presets enable selected constraints. They do not establish compliance with a company standard or import an installed-equipment rating merely because the method name mentions that standard. Inspect every enabled constraint and explicitly configure ratings from the applicable design basis. Mechanical-design database lookups are a separate workflow.

### 23.4.6 Equipment-Specific Constraints

Table 23.1 summarizes the constraints defined by each equipment type.

**Table 23.1.** Engineering checks to consider by equipment type. This is a checklist, not a promise that every listed check is created automatically; discover the actual registered constraint names and coverage.
...
The Pareto front reveals the trade-off between objectives, allowing the engineer to make an informed decision about where to operate. The engineer must select a preferred point using declared economic or operational preferences. The current `ParetoResult` exposes the front, utopia and nadir points; it does not provide an automatic knee-point method. A finite weighted-sum sweep can miss unsupported portions of a non-convex front.
...
# minimum outlet pressure is a feasibility requirement, not a compressor setter.
...
print(f"Returned throughput: {engine_result.getOptimalValue():.0f} kg/hr")
# Replay explicitly for this engine before reading equipment outputs.
feed.setFlowRate(engine_result.getOptimalValue(), "kg/hr")
process.run()
assert engine_result.isConverged(), "Rejected capacity-search candidate"
capacity_status = [(str(item.getEquipmentName()), float(item.getUtilization()),
                    bool(item.isWithinLimits())) for item in
                   engine.evaluateAllConstraints().getEquipmentStatuses()]
print("Native final candidate accepted:", all(item[2] for item in capacity_status))
# Independently enumerate the declared domain and retain a checked grid maximum.
import numpy as np
engine_grid = []
for trial in np.linspace(min_flow, max_flow, 31):
    feed.setFlowRate(float(trial), "kg/hr")
    feed.setPressure(inlet_pressure, "bara")
    process.run()
    within = all(item.isWithinLimits() for item in
                 engine.evaluateAllConstraints().getEquipmentStatuses())
    engine_grid.append((float(trial), bool(within)))
accepted_engine_rate = max(rate for rate, within in engine_grid if within)
feed.setFlowRate(accepted_engine_rate, "kg/hr")
process.run()
assert all(item.isWithinLimits() for item in
           engine.evaluateAllConstraints().getEquipmentStatuses())
print("Independently checked grid maximum kg/hr:", accepted_engine_rate,
      "grid spacing kg/hr:", (max_flow-min_flow)/30)
print("Nominal-capacity diagnostics:", list(engine_result.getConstraintViolations()))
```

The source engine returned about126,510 kg/h, but its constraint strategy rejected that final state. A separate31-point rate grid retained125,000 kg/h with all configured hard checks satisfied. This is the largest accepted sampled rate with5,000 kg/h spacing; neither the convergence flag nor the diagnostic text is an independent feasibility certificate.
...
**generateCapacityScreening()** — Sweeps supported pressure and temperature axes for a fixed composition to calculate a mass-throughput screening table:
...
# A NaN from the native search is rejected. Supply a separate physical grid screen.
capacity_grid = []
for point in capacity_table.getPoints():
    accepted_rates = []
    for trial in np.linspace(50000.0, 200000.0, 21):
    ...
    assert accepted_rates
    capacity_grid.append({"pressure_bara": float(point.getInletPressure()),
        "native_candidate_kg_hr": float(point.getMaxFlowRate()),
        "grid_maximum_kg_hr": max(accepted_rates), "grid_spacing_kg_hr": 7500.0})
    print(capacity_grid[-1])
```

For this pinned model, the native screening search returns missing values at50 and60 bara. Those values are rejected, while the separate21-rate physical sweeps supply accepted sampled capacities. The finite70-bara native point is also subjected to physical replay; the displayed grids use7,500 kg/h spacing and do not represent well BHP.


This table maps declared process boundaries to a screened maximum mass throughput. It is not a well VFP table: a reservoir export requires independently solved BHP values, a datum and phase-volume bases. The legacy composition arguments accept only singleton zero placeholders at this source revision.

### 23.8.3 Equipment Capacity Strategies

Internally, the `ProcessOptimizationEngine` uses an `EquipmentCapacityStrategyRegistry` with registered built-in strategies that know how to evaluate capacity for specific equipment types (separators, compressors, valves, heat exchangers, pipelines, etc.). When the engine evaluates constraints, it looks up the appropriate strategy for each equipment item and delegates the capacity calculation.
...
print("Surge check enabled:", surge_constraint.isEnabled(),
      "disabled-chart API sentinel:", surge_constraint.getCurrentValue())
```
...
The integration evaluates the supplied map. A generated map remains an assumed screening model; installed performance requires vendor or measured curves, gas-property corrections and a verified operating envelope.
...
Available templates include `CENTRIFUGAL_STANDARD`, `CENTRIFUGAL_HIGH_FLOW`, and `CENTRIFUGAL_HIGH_HEAD`, each representing typical performance characteristics for different compressor designs. These templates provide assumed curve shapes for concept-level sensitivity studies; this chapter does not validate them against a particular compressor.
...
Corrections are model options whose applicability must be checked against measured or vendor data. Extrapolation away from the reference map is not validated by enabling them.
...
Internally, `AgenticProcessOptimizer` runs a **bounded Nelder\u2013Mead simplex** with deterministic, seeded random initialization. A flowsheet behind `evaluate()` can be treated as a feasibility-gated black box. Derivative-free search is one option; finite-difference or analytic sensitivities can be preferable when the model is smooth and reproducible. The same seed and the same problem produce an identical trajectory, which makes experiments reproducible. Address-based constraints enter the score as weighted quadratic penalties; finite penalties alone are not hard feasibility guarantees:
...
`getReadinessJson()` rates each capability `full`, `partial`, or `none`. The never-throwing JSON contract, deterministic seeding, bounded action space, reward shaping, constraint handling, trajectory logging, and feasibility gating are all rated `full`; gradient information is `none`; the readiness JSON describes global-search capability as `partial`, but this is not a mathematical guarantee of a global optimum; and parallel evaluation is `none`. This self-rating lets an autonomous planner decide whether `AgenticProcessOptimizer` is the right tool for a given problem or whether a global or gradient-based method should be used instead.
...
![Figure 23.2: Conceptual interaction among process, fluid, equipment, objective, automation and result roles](figures/fig25_2_component_diagram.png)

<!-- scientific-illustration:fig25_2_component_diagram.png -->
Boxes denote software roles, and some labels are abstractions rather than exact class names. Use the executable API examples and recorded source revision for concrete class and method contracts.
<!-- /scientific-illustration -->
...
![Figure 23.3: Conceptual sequence linking decisions, full-model solution, evidence checks and final-point replay](figures/optimization_evidence_loop_2026.svg)
...
![Figure 23.4: Equipment Utilization vs Feed Rate](figures/ch25_utilization_vs_rate.png)
...
![Figure 23.5: Production Feasibility Curve](figures/ch25_feasibility_curve.png)
...
5. **Custom objectives and constraints** extend the framework beyond throughput maximization to multi-objective optimization combining production, energy, and emissions targets. Sampled Pareto fronts support an explicit preference-based decision; no unique knee need exist.
...
7. **CompressorChartGenerator** integration evaluates the supplied compressor-map model, including surge margin enforcement and operating point tracking.
...
**Exercise 23.7.** *Pareto front generation.* For the process with export compressor, define two competing objectives: maximize throughput and minimize specific energy consumption (kWh/tonne of product). Generate a 15-point Pareto front using `optimizePareto()`. Plot the sampled front, state the objective scaling and test whether a defensible knee exists. What is the throughput penalty for operating at minimum specific energy versus maximum throughput?
...
7. API RP 14E (1991) *Recommended Practice for Design and Installation of Offshore Production Platform Piping Systems*, 5th edn, American Petroleum Institute.
