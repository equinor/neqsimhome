...
$$
(A_{21} A_{11}^{-1} A_{12}) \Delta H = A_{21}A_{11}^{-1}r_1-r_2
$$
...
$$
\frac{d\Delta P}{dQ} = \frac{1}{\text{PI}} \quad \text{(oil)} \qquad \frac{d\Delta P}{dQ} = \frac{1}{2P_{wf}\,\text{PI}_{gas}} \quad \text{(gas)}
$$
...
Physical gas lift mixes injected gas with production, changing density, friction, temperature and total mass. In this network's legacy gas-lift switch, however, the pressure benefit is only a capped loss proxy:
...
A physical production gain must be established from the coupled IPR, tubing, injection pressure and downstream constraints. Do not interpret a gain from this capped proxy as an empirical50–200% recovery prediction.
...
$$
\Delta P_{ESP}=\rho g H_{stage}N_{stages},\qquad P_{shaft}=Q_{vol}\Delta P_{ESP}/\eta
$$
...
For this fixed-source-pressure example, the gas chokes remain in the supported critical-flow regime. Total plant flow at 40%,60%,80% and 100% opening is6.11035,7.37624,8.64213 and 9.90802 kg/s. The largest hydraulic residual is11.45 Pa and the mass residual is below 1e-6 kg/s. The approximately linear increase is the expected critical-capacity response; this model contains no reservoir IPR and cannot demonstrate a reservoir-limited plateau. The opt-in forward gas-capacity model is essential: the legacy head-loss choke approximation can report hydraulic convergence while flagging unsupported critical flow.
...
3. **Adaptive relaxation** can reduce oversized steps; nonsmooth or infeasible cases can still fail
...
- Adaptive relaxation can improve robustness but does not guarantee convergence
...
# Restore
network.getPipe("Choke-B").setChokeOpening(original_opening)
network.run()
```
...
![Figure 6.1: Choke Sensitivity Study: Pressure vs Choke Opening](figures/ch15w_choke_sensitivity.png)
...
![Figure 6.2: Wellhead vs Manifold Arrival Pressure](figures/ch15w_well_network_comparison.png)
...
- **The NR-GGA solver** reduces the network system through a Schur complement. Accept a candidate only after checking hydraulic residuals, nodal mass balance and every element’s supported model status; runtime alone does not establish suitability.
- **Artificial lift switches** provide screening pressure boosts. The current network gas-lift switch reduces computed loss using a capped gas/produced-mass ratio; it does not inject a material stream or predict the full hydrostatic/friction optimum. Use the explicit lift-gas mixer and tubing workflow of Chapter5 for component and energy balances.
