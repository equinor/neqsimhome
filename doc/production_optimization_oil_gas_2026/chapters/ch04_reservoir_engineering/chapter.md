...
For pseudo-steady flow in a closed circular drainage area, use average reservoir pressure $\bar P$ rather than the boundary pressure $P_e$. In field units (STB/day, mD, ft, psi, cP):

$$
q_o = \frac{0.00708\, k h (\bar P - P_{wf})}{B_o \mu_o \left[\ln\left(\frac{r_e}{r_w}\right) - 0.75 + S\right]}
$$
...
$$
P_{wf} = P_i - \frac{q B \mu}{4\pi k h}\left[\ln\left(\frac{4 k t}{\phi \mu c_t r_w^2}\right) - \gamma + 2S\right]
$$
...
Here $m$ is the positive magnitude of the Horner straight-line slope; the plotted pressure decreases with increasing logarithmic Horner ratio. The skin factor is:
...
The extrapolation of the Horner straight line to $\frac{t_p + \Delta t}{\Delta t} = 1$ (i.e., infinite shut-in time) gives the extrapolated $P^*$. Under the constant-rate, infinite-acting homogeneous model it estimates the undisturbed reference pressure; in a depleted bounded reservoir it is not automatically the current volume-average pressure. Boundary corrections and production history are required for that interpretation.

![Figure 4.1: Analytical Horner buildup illustration with increasing shut-in time toward the right](figures/horner_plot.png)

<!-- scientific-illustration:horner_plot.png -->
...
- **Wellbore storage:** Formation fluid continues to enter and compress the wellbore inventory after surface shut-in of a producer, masking the early-time reservoir response. The wellbore storage coefficient is $C = V_w c_w$ for a liquid-filled wellbore.
...
![Figure 4.2: Analytical PI, Vogel and Fetkovich inflow curves with all assumed parameters stated](figures/ipr_curves.png)

<!-- scientific-illustration:ipr_curves.png -->
...
N_p[B_o + (R_p-R_s)B_g]
 ...
 &\quad + N\frac{(1+m)B_{oi}(c_w S_{wi}+c_f)}{1-S_{wi}}\Delta P \\
...
$$
E_{fw} = \frac{(1+m)B_{oi}(c_w S_{wi} + c_f)}{1 - S_{wi}} \Delta P
$$
...
- **Volumetric undersaturated oil** ($m=0$, $W_e=0$, above $P_b$): $F=N(E_o+E_{fw})$, where $E_o=B_o-B_{oi}$; oil expansion must be retained
...
$$
U = 2\pi f\phi_a c_t h_a r_a^2
$$

Use SI consistently: permeability $k_a$ in m², time in s, viscosity in Pa s, compressibility in Pa$^{-1}$, and lengths in m; $U$ then has units m³/Pa. The symbol $k_a$ here is permeability, distinct from the preceding steady-influx coefficient. Here $f$ is the fraction of the aquifer circle (1 for full encirclement, 0.5 for half, etc.).
...
- Under the volumetric, isothermal, closed-tank assumptions, $G-G_p$ is remaining gas in place on the same standard-volume basis; recoverable reserves additionally require an abandonment and commercial-development assessment
...
![Figure 4.3: Analytical straight-line p/Z material-balance illustration for a specified volumetric gas reservoir](figures/pz_plot.png)

<!-- scientific-illustration:pz_plot.png -->
The straight line is an assumed idealized material-balance relation. Its intercept and abandonment cut-off illustrate how resource and recovery estimates differ; real interpretation needs consistent standard-volume data, reservoir-average pressure, Z and the reservoir drive assumptions.
<!-- /scientific-illustration -->
...
![Figure 4.4: Checked analytical nodal intersections with fixed hydrostatic head and diameter-dependent friction](figures/nodal_analysis.png)
...
![Figure 4.5: Illustrative exponential, hyperbolic and harmonic rate histories over 20 years](figures/decline_curve.png)

<!-- scientific-illustration:decline_curve.png -->
The curves compare specified Arps forms and an assumed economic cut-off. They show rate, not cumulative production, and are not history-matched resource or reserve estimates.
<!-- /scientific-illustration -->
...
The productivity reduction depends on condensate saturation, relative permeability, capillary number and the near-well pressure/composition path; no universal 50–80% PI penalty follows from crossing the dew point. A fixed single-phase IPR does not resolve this mechanism. Accurate modeling requires compositional simulation with relative permeability effects.
...
- **P90 / low estimate:** at least90% probability of equaling or exceeding the stated recoverable quantity; corresponds to 1P (Proved) when all reserves criteria are met.
- **P50 / best estimate:** at least50% exceedance probability; corresponds to 2P (Proved plus Probable), not Probable alone.
- **P10 / high estimate:** at least10% exceedance probability; corresponds to 3P (Proved plus Probable plus Possible), not Possible alone.

A simulated quantity distribution does not establish commerciality or reserves status. Apply the complete PRMS project and economic criteria.\cite{foundationPRMS2018}
...
![Figure 4.6: Inflow Performance Relationship (IPR) Curves. Analytical linear-PI and Vogel inflow relationships](figures/ch04_fig01_ipr_curves.png)
...
![Figure 4.7: Gas-Oil Ratio vs Depletion Pressure at Reservoir Temperature](figures/ch04_fig02_gor_vs_pressure.png)
...
![Figure 4.8: Fluid Density and Temperature vs Depth. NeqSim properties evaluated along prescribed temperature and pressure gradients](figures/ch04_fig03_density_vs_depth.png)
...
![Figure 4.9: Relative Permeability Curves (Corey Model). Analytical Corey relative-permeability model](figures/ch04_fig04_rel_perm_curves.png)
...
The decline cumulative-volume derivative and pressure/unit relations are checked independently. The reservoir objects demonstrate a lumped inventory/API boundary; they do not validate spatial reservoir sweep or commercial reserves. The P/Z interpretation is restricted to its closed, isothermal volumetric assumptions, and the Horner extrapolation is distinguished from average pressure in a bounded depleted reservoir.\cite{foundationPRMS2018}

The calculation and literal-code records are in `verification/scientific_revision/ch04_manuscript_physics.json`; the chapter scope and code hashes are indexed in `foundations_review.json`.
