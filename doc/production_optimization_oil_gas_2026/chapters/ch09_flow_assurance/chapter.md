...
![Figure 9.1: Flow assurance threats in a subsea production system](figures/flow_assurance_threats.png)
...
- $\Delta\mu_w^H=\mu_w^{empty}-\mu_w^{hydrate}$ is the positive empty-lattice minus occupied-lattice reference difference in this equation
...
The following calculation tabulates hydrate-equilibrium temperature against pressure for the stated composition. Compare these values with a separately calculated operating path and its uncertainty; this example does not plot an inhibited boundary or establish blockage probability.
...
Subcooling measures the temperature driving force relative to the calculated hydrate-equilibrium boundary. Blockage risk also depends on free-water availability, kinetics, residence time, flow history and inhibition; subcooling alone does not determine its severity:

$$
\Delta T_{sub} = T_{hydrate}(P) - T_{operating}
$$

The following bins are illustrative screening categories, not validated blockage-risk levels or universal intervention requirements. Select mitigation using the fluid, water and operating-history evidence for the case.

| Subcooling $\Delta T_{sub}$ | Illustrative category | Example mitigation to assess |
|------------------------------|-----------|-----------------|
| $< 0$ °C | Outside the calculated hydrate-stability region | Check uncertainty and possible cooling or pressure transients |
...
- Can be recovered by appropriate separation/distillation, but losses and recovery economics often favor once-through injection
...
- $K_H$ is the Hammerschmidt constant (1,297 for both MEG and methanol with depression in °C;2,335 belongs to the °F convention)
...
| Effective subcooling | Limited by equilibrium, concentration and freeze/viscosity envelope | Limited by equilibrium, concentration and losses | Product/fluid/time-dependent test envelope | Product/fluid/slurry-transport test envelope |
| Watercut limit | None | None | None | < 50–60% |
| Hold time | Requires retained inhibitor concentration | Requires retained inhibitor concentration | Qualified induction/hold-time tests | Qualified agglomeration, settling and restart tests |
| Shut-in protection | Verify concentration along the cooling path | Verify concentration and phase losses | Verify qualified hold time | Verify settling and restart behavior |
| Recovery/regeneration | MEG regeneration/reclamation | Recovery possible where economical | Usually once-through | Usually once-through |
...
| Chemical inhibitors | PPDs modify crystal morphology/network formation | Can improve pour point and yield stress; do not assume a lower equilibrium WAT |
...
- **Pressure depletion** above the bubble point can reduce asphaltene solubility; below the bubble point, gas liberation changes the liquid composition and can promote redissolution
...
An upper onset can lie above bubble pressure, while a lower redissolution boundary may lie below it. Bubble pressure is not itself the lower precipitation boundary. Determine both onsets along the actual composition and temperature path, then assess deposition kinetics, transport, residence time and tested mitigation before choosing an operating envelope.
...
At higher dispersed-phase fractions approaching the inversion point, the viscosity rises steeply. The **Krieger–Dougherty suspension relation** captures this behavior:

$$
\mu_{em} = \mu_c \cdot \left(1 - \frac{\phi}{\phi_{max}}\right)^{-2.5\phi_{max}}
$$

where $\phi_{max}$ is a fitted maximum packing fraction. The exponent uses the rigid-sphere intrinsic viscosity2.5; liquid droplets can deform and exchange interfacial stress, so this is an empirical emulsion approximation rather than a universal droplet law. Inversion also depends on wetting, shear history and phase viscosity.
...
- $E$ is the eroded-target mass per impacting-sand mass [kg/kg], not a rate
...
- $\dot m_p$ is the impacting-sand mass rate [kg/s] used below
...
$$
E_r = \frac{K \cdot F(\alpha) \cdot v_p^n \cdot \dot m_p \cdot G}{A_t \cdot \rho_t}
$$

where $A_t$ is the target area exposed to particle impacts [m²], $\rho_t$ is the target material density [kg/m³], and $G$ is a geometry factor that accounts for the flow pattern concentration at the impact zone. Geometry and material coefficients must come from a specified erosion model and tests with consistent velocity units, impact angle, sand size and particle properties. The former universal material-coefficient and geometry-factor tables lacked that basis and cannot support a wall-loss calculation. A geometry that reduces erosion in one flow regime can increase it in another.
...
- **Geometry optimization** — compare bends, swept tees, blind tees and replaceable target elements using a calibrated model or relevant testing. Blind tees can protect some impact locations through particle redirection or a retained sand cushion, but can concentrate wear elsewhere; no universal 50% reduction or blanket prohibition follows from geometry alone
...
This screening example calculates a hydrate curve and lists assumed pipeline states for comparison. The raw saturation trace, hydrate calculation and assumed operating points retain separate acceptance limits; they do not define a comprehensive safe operating window.
...
The raw saturation-trace call in this fragment is **not accepted as a complete physical envelope**. It demonstrates candidate generation; finite points alone do not establish branch identity or specification limits. The hydrate calculations have separate finite-domain checks; the listed pipeline conditions are assumed overlay points. Those checks do not validate the saturation trace.
...
For the conventional field-unit coefficient $C$, this displayed equation gives ft/s when density is lb/ft³. In SI use $v_e=1.2193C/\sqrt{\rho_m}$ in m/s with density in kg/m³. The C-factor and any operating fraction are service assumptions; this screen does not establish sand erosion, corrosion or vibration acceptance.

**Flow-induced vibration (FIV)** includes low-frequency structural response to turbulent or multiphase excitation. **Acoustic-induced vibration (AIV)** is a distinct high-frequency shell response often associated with high-energy gas pressure reduction. Velocity alone does not distinguish or accept either mechanism. The Energy Institute *Guidelines for the Avoidance of Vibration Induced Fatigue Failure in Process Pipework* provides screening criteria based on the **Likelihood of Failure (LOF)** and the RMS dynamic force at branch connections.
...
![Figure 9.2: Hydrate Equilibrium Curve — Natural Gas with Water](figures/fig01_hydrate_equilibrium.png)
...
![Figure 9.3: Hydrate Temperature vs MEG (P = 100 bara)](figures/fig02_meg_inhibition.png)
...
![Figure 9.4: Water dew point at 50 ppmv water](figures/fig03_water_dewpoint.png)
...
![Figure 9.5: Pipeline Operating Envelope with Hydrate Boundary. Calculated hydrate equilibrium with assumed normal-operation and shutdown paths](figures/fig04_hydrate_operating_envelope.png)
...
The hydrate results are screened for finite temperature and the expected direction of MEG inhibition. Hammerschmidt concentration/depression conversions are checked against the corrected SI constant, while solid formation, deposition kinetics, corrosion and erosion require model-specific calibration beyond these examples. Phase equilibrium and a finite onset do not imply a safe shutdown time or an inhibitor guarantee.\cite{foundationGPSAerrata,deboer1995}

The calculation and literal-code records are in `verification/scientific_revision/ch09_manuscript_physics.json`; the chapter scope and code hashes are indexed in `foundations_review.json`.
...
3. **Exercise 9.3:** Using the de Waard-Milliams model, calculate the CO$_2$ corrosion rate as a function of temperature (20–120°C) for CO$_2$ partial pressures of 1, 5, and 20 bar. Plot the results and show that this uncorrected relation increases monotonically with temperature at fixed CO$_2$ pressure. Explain why protective-film, pH and transport effects are needed to predict a nonmonotonic field trend.
