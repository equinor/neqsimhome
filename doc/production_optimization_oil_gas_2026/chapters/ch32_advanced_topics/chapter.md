...
| Application decision rule | Select among sampled non-dominated points using declared economics or preferences; no `getKneePoint()` API is supplied |
...
Curvature depends on objective scaling and on parameterization. A knee can be useful when a curved front has a clearly stated normalization, but is not automatically the best compromise.

The current NeqSim result does not provide an automatic knee-point helper. A caller can normalize the two objective ranges, compute each sampled point's perpendicular distance from the line connecting the extremes, and examine the maximum-distance point as one possible preference rule. This construction depends on the chosen normalization and is undefined as a unique preference when all sampled points lie on a straight line.
...
|-----------|-----------|-------------------|
| Oil production rate | Maximize | Stabilized oil product volume at declared stock-tank conditions |
...
| Gas dew point | Minimize | Explicit dew-point flash on the gas composition at stated pressure |
...
The weighted-sum method has the advantage of simplicity and can use a suitable single-objective optimizer; strictly positive weights avoid zero-weight ties that can include weakly efficient points. However, it has a well-known limitation: it cannot find points on non-convex portions of the Pareto front.
...
A suitable epsilon grid can recover nonconvex portions that weighted sums miss, provided the subproblems are solved adequately. A finite grid returns sampled non-dominated states, not a guarantee of the complete continuous front.
...
# Independent solved rate grid: weighted linear objectives can return sparse endpoints.
grid_production = np.linspace(50000.,200000.,25)
grid_power = []
for rate in grid_production:
    fresh_fluid = fluid.clone()
    fresh_feed = jneqsim.process.equipment.stream.Stream("Replay feed",fresh_fluid)
    fresh_feed.setFlowRate(float(rate),"kg/hr")
    fresh_feed.setTemperature(40.,"C");fresh_feed.setPressure(50.,"bara");fresh_feed.run()
    fresh_comp = jneqsim.process.equipment.compressor.Compressor("Replay compressor",fresh_feed)
    fresh_comp.setOutletPressure(150.,"bara")
    fresh_comp.setUsePolytropicCalc(True);fresh_comp.setPolytropicEfficiency(.78);fresh_comp.run()
    fresh_feed.getFluid().initProperties();fresh_comp.getOutletStream().getFluid().initProperties()
    work = float(fresh_comp.getPower("kW"))
    dh = (fresh_comp.getOutletStream().getFluid().getEnthalpy()
          - fresh_feed.getFluid().getEnthalpy())/1000.
    assert abs(fresh_comp.getOutletStream().getFlowRate("kg/hr")-rate)<1e-5
    assert abs(dh-work)<1e-5*work
    grid_power.append(work)
assert np.all(np.diff(grid_power)>0)
plt.figure(figsize=(7, 4))
plt.plot(grid_production,grid_power,".-",label="Independent full-model grid")
plt.scatter(production, power,label="Native weighted-search samples",zorder=3)
...
<!-- additional-scientific-literal-figure:ch32_advanced_topics -->

![Figure 32.1: Twenty-five independently solved rate cases compared with the native weighted-search sample.](figures/ch32_literal_pareto_grid.png)

**Discussion (Figure 32.1).** At fixed 50→150 bara compression and 40 °C suction, the independent grid spans 50,000–200,000 kg/h and 3250.84–13003.37 kW. Its monotonic linear trade-off has no unique knee or preferred throughput without a capacity or economic criterion. The native weighted search returns one endpoint near 200,000 kg/h, which agrees with the independently solved grid. Each grid point passes mass and shaft-work/enthalpy checks; this comparison establishes the sampled model trade-off, not a vendor-map or field validation.


This produces a scatter plot of the Pareto front, clearly showing the trade-off between production rate and energy consumption. At fixed inlet state and constant efficiency, power is approximately proportional to mass flow, so this example has an almost straight trade-off and no physically privileged knee. A sampled geometric heuristic does not supply economic preferences.
...
supports globalization under additional regularity and algorithmic assumptions; it does not prove convergence to a global minimum. The penalty parameter $\sigma$ is adjusted dynamically.
...
Under a constraint qualification, KKT conditions are necessary for a differentiable local optimum. They are sufficient for global optimality for a convex minimization problem with convex inequality functions written as $g\leq0$ and affine equalities; nonconvex problems need additional second-order sufficiency conditions. Constraint qualification alone is not sufficient. Bound multipliers must also be included when bounds are active \cite{boyd2004convex}.
...
### 32.4.2a Accepted compressor optimum with an independent comparison
...
When external optimizers require gradient information, central finite differences provide the following general approximation:
...
```java
// Copy the definition into standard JSON values; unbounded limits become null.
...
Before performing data reconciliation, it is essential to verify that the plant is at steady state. Steady-state balance reconciliation is inappropriate when unmodeled accumulation is significant; dynamic reconciliation is a different formulation. NeqSim provides the `SteadyStateDetector` class based on the R-statistic method:

The R-statistic for a time series $\{y_1, y_2, \ldots, y_N\}$ is:

$$
R = \frac{\sum_{k=1}^{N-1}(y_{k+1}-y_k)^2}{2(N-1)s_y^2}
$$

where $\bar{y}$ is the mean and $s_y^2$ is the variance. For independent, stationary finite-variance noise, $R$ is near one; slow trends often reduce it. The native implementation handles negligible sample variance separately. Autocorrelation, oscillation and sensor freeze require additional checks; this ratio alone does not prove steady state.
...
$$
z_i = \frac{\hat{y}_i-y_i}{\sqrt{V_{ii}-V^{\mathrm{adj}}_{ii}}}
$$

For full-row-rank linear constraints, $V^{\mathrm{adj}}=V-VA^T(AVA^T)^{-1}AV$. This is the reconciled covariance, so the denominator is the standard deviation of the **adjustment**. The native method uses this expression. A two-sided Gaussian screen uses $z_{1-\alpha/2}$ (1.96 for an individual 95% test). Multiple tests, correlated errors and uncertain model constraints affect false-alarm rates; a flag is evidence for investigation, not proof of a faulty sensor. A zero adjustment variance provides no identifiable residual test.
...
|-----------|--------|-------|-------------|
| Full factorial | `.vary(...).build().run()` | $\prod n_i$ | All combinations of parameter values |
| One-at-a-time | Application-built parameter cases | $\sum (n_i - 1) + 1$ | Vary each parameter individually |
| Latin Hypercube | External LHS sampling and explicit cases | $N$ | Space-filling design |
| Random | External seeded sampling and explicit cases | $N$ | Random sampling within bounds |
...
$$
\text{s.t.} \quad p_w^{\text{res}} - q_w/J_w = p_w^{\text{bh}} \quad \forall w \in \mathcal{W} \quad \text{(IPR)}
$$

$$
p_w^{\text{bh}} - \Delta p_{w,m}(q_w) = p_m \quad \forall (w,m) \quad \text{(flowline pressure drop)}
$$
...
where $q_w$ is the flow rate from well $w$, $p_w^{\text{res}}$ is the reservoir pressure, $J_w$ has units of rate per pressure difference. The linear IPR is an illustrative single-phase approximation. The bottomhole-to-manifold pressure change includes tubing hydrostatics, friction, acceleration and the surface flowline; do not apply reservoir drawdown directly to wellhead pressure.

This is a nonlinear optimization problem because the pressure drops are nonlinear functions of flow rate (for example, Darcy–Weisbach for a qualified single-phase pipe or Beggs–Brill for its empirical multiphase scope).
...
where $N$ is the total number of unknowns and nnz is the number of non-zero entries. Sparse storage can be $O(N)$ for bounded-degree network equations, but solve time depends on topology, fill-in, ordering and conditioning. Neither sparse LU nor GMRES has a general $O(N)$ cost guarantee for this network model.
...
for i, (gor, wc) in enumerate([(200, 0.1), (350, 0.25), (150, 0.05)]):
    fluid_i = jneqsim.thermo.system.SystemSrkCPAstatoil(273.15 + 80, 250.0)
...
For differentiable concave gas-lift curves and an active shared-gas constraint, wells strictly inside their individual bounds have equal marginal oil gain per unit lift gas. Wells at zero flow or another bound satisfy the corresponding KKT inequality instead. This is the equal-slope principle:
...
# Scale both decisions and objective to avoid false termination in raw flow units.
result = minimize(lambda y: total_oil(y*Q_gl_available)/10000.0,
    np.array(x0)/Q_gl_available, method='SLSQP', bounds=[(0.0, 1.0)]*3,
    constraints={'type': 'ineq', 'fun': lambda y: 1.0-sum(y)},
    options={'ftol': 1e-12, 'maxiter': 200})
result.x = result.x * Q_gl_available
result.fun = result.fun * 10000.0
assert result.success and sum(result.x) <= Q_gl_available + 1e-4
...
The variables and objective are scaled before SLSQP termination is evaluated. The accepted objective is7,935.412 bbl/day and agrees within0.01 bbl/day with an independently solved common-marginal (KKT) allocation. These concave curves are declared algebraic examples, not calibrated NeqSim well deliverability.
...
![Figure 32.2: Production and compression-power trade-off. Thirty full NeqSim compressor candidates](figures/ch22_pareto_front.png)
...
![Figure 32.3: Seasonal operation: process and utility comparison](figures/ch22_scenario_comparison.png)
...
2. **Sequential Quadratic Programming** via `SQPoptimizer` provides fast, efficient solving of constrained nonlinear programs with equality and inequality constraints, BFGS Hessian updates, and reported KKT residuals that require interpretation
...
The physics residual term $\mathcal{L}_{\text{residual}}$ is computed using automatic differentiation, evaluating derivatives of the differentiable network to floating-point precision. This does not automatically differentiate an external Java flash solver or eliminate discretization, optimization and model error. The key advantage is that PINNs work even with sparse observational data — the physics residual provides supervisory signal everywhere in the domain.
...
A finite residual penalty encourages conservation at sampled points; it does not enforce exact balances or guarantee extrapolation. Test integral and component balances on independent cases and near phase boundaries. Hard conservative architectures or explicit projections require their own analysis \cite{raissi2019pinn}.
...
$$
r_t = w_{\text{prod}} \cdot Q_t - w_{\text{energy}} \cdot P_{\text{comp},t} - w_{\text{penalty}} \cdot \sum_{i} \max(0, u_{i,t} - 1) - w_{\text{balance}} \cdot \sigma(\mathbf{u}_t)^2
$$
...
The penalty term uses a ReLU-style formulation: zero when all equipment operates within capacity ($u_i \leq 1$), and linearly increasing for violations. The hinge penalty is continuous but nondifferentiable at the limit, and a finite reward penalty does not guarantee feasibility. Keep hard feasibility rejection outside the reward and scale all reward terms consistently. Penalizing loading variance is optional: equal utilization is not generally the economic optimum.
...
- **High separator liquid utilization** ($u > 0.95$): Reduce liquid loading by choking back high-water-cut wells, or test a different separation pressure and recompute gas/liquid loads; increasing pressure at fixed temperature generally suppresses vaporization
...
The constraint-penalty reward of Section 32.13.3 guides search but does not keep the agent feasible; the *driving* term of the reward should be monetary. NeqSim provides `EconomicParameters` and `ValueChainObjective` (package `neqsim.process.optimization.valuechain`) to convert a flowsheet's export rates and power draw into a single net-value-per-day signal that already accounts for energy cost and carbon:
...
An ensemble size such as 5–10 is an experimental design choice, not evidence of calibrated uncertainty. Verify empirical coverage and interval width on independent states; correlated model errors can leave ensemble spread small despite large shared bias. At inference time, all ensemble members are evaluated and the spread of predictions directly communicates model confidence.
...
For production operations, the choice between these formulations depends on the consequence of constraint violation. Declared hard safety and design limits remain hard acceptance conditions. A probability estimate from a surrogate does not authorize their relaxation. Robust/chance-constrained studies can quantify uncertainty only under a separately justified model, uncertainty set and risk criterion. Economic constraints (throughput targets) can tolerate expected-value optimization.
...
- **Consistency**: Structured schemas and executable checks can reduce setup variability, but LLM-generated configurations still need review and validation; the interface does not guarantee correct physics or units.
...
## 32.16 Measuring computational requirements
...
15. Schweidtmann, A.M., Huster, W.R., Lüthje, J.T., and Mitsos, A. (2019). Deterministic global process optimization: Accurate (single-species) properties via artificial neural networks. *Computers & Chemical Engineering*, 121, 67–74. DOI: [10.1016/j.compchemeng.2018.10.007](https://doi.org/10.1016/j.compchemeng.2018.10.007).
