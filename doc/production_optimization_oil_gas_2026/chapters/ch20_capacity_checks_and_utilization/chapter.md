...
![Figure 20.1: Conceptual gas/liquid capacity trade-off and assumed operating points](figures/separator_capacity_diagram.png)

<!-- scientific-illustration:separator_capacity_diagram.png -->
The boundary is an illustrative constraint shape, not a computed gas-velocity field or liquid-retention design. A real envelope needs phase densities, vessel geometry, internals, level and both gas and liquid performance limits.
...
![Figure 20.2: Analytical compressor-map illustration with explicit surge and high-flow endpoints](figures/compressor_capacity_map.png)

<!-- scientific-illustration:compressor_capacity_map.png -->
...
# Two solved conductances; no arbitrary assumed 50 K driving force.
import numpy as np
UA_design = 50000.0
...
![Figure 20.3: Assumed individual equipment capacities on one common production basis](figures/capacity_staircase.png)

<!-- scientific-illustration:capacity_staircase.png -->
Bars compare independent equipment ceilings at the same reference production basis. They do not represent a sequential series of completed debottlenecking projects.
<!-- /scientific-illustration -->
...
This reduced worked example checks HP separation, one compression stage, a specified-temperature cooler and an export pipeline. The oil and water outlets leave at the HP separator boundary. It demonstrates declared operating screens rather than a complete installed-facility rating.
...
- **Export Compressor** (K-100): One modeled compression stage with specified polytropic efficiency
- **Gas Cooler** (E-100): Outlet fixed at 40 °C; external duty is calculated, with no exchanger UA or cooling-water circuit modeled
...
pipeline.setLength(80000.0)  # 80 km; API length is metres
...
print(f"    Linear screening extrapolation: {max_production:.0f} kg/hr "
...
Utilization curves can reveal the following features when the model and constraints support them. The literal separator/compressor rate sweep has fixed composition, pressure and temperature, so its two utilizations scale proportionally and need not cross:
...
- **The capacity limit**: The production rate at which any equipment reaches 100% utilization. This is a limit for the included screens only; omitted equipment and nonlinear interactions can impose a lower facility limit.
...
![Figure 20.4: Unclipped utilization for three explicitly assumed independent equipment ratings](figures/utilization_vs_production.png)

<!-- scientific-illustration:utilization_vs_production.png -->
...
Changing water cut affects both liquid handling and gas compression through the chosen rate and composition basis. The following executed methane/heptane/water scenario fixes total reference oil-plus-water volume and associated-gas volume per unit oil. Gas input therefore decreases as water replaces oil; no universal trend is imposed on equipment utilization:

```python
# Executed water-cut scenario on an explicit common liquid reference basis.
...
| Strategy | Effect | Capacity qualification |
|----------|--------|--------------------------|
| Upgrade internals (mesh to vane pack) | Increases $K_{\text{SB}}$ | Requires unit and whole-process re-rating |
| Install cyclonic inlet device | Better inlet separation | Requires unit and whole-process re-rating |
...
| Add parallel separator | Adds nominal installed capacity | Requires unit and whole-process re-rating |
...
| Strategy | Effect | Capacity qualification |
|----------|--------|--------------------------|
| Suction pressure optimization | Reduces compression ratio, saves power | Requires unit and whole-process re-rating |
| Intercooler improvement | Lower interstage temperature, less power | Requires unit and whole-process re-rating |
| Impeller re-wheel | Changed characteristic | Variable |
| Add parallel compressor | Adds nominal flow capacity | Requires unit and whole-process re-rating |
| Driver upgrade | Higher power available | Variable |
| Anti-surge valve optimization | Reduced recycle flow | Requires unit and whole-process re-rating |

### 20.11.3 Heat Exchanger Debottlenecking

| Strategy | Effect | Capacity qualification |
|----------|--------|--------------------------|
| Online cleaning | Restores U to design value | Requires unit and whole-process re-rating |
| Tube insert (turbulator) | Increases tube-side h | Requires unit and whole-process re-rating |
| Enhanced tubes (finned, twisted) | Higher heat transfer area | Requires unit and whole-process re-rating |
| Add parallel exchanger | Adds nominal exchanger capacity | Requires unit and whole-process re-rating |
| Increase cooling medium flow | Improves temperature approach | Requires unit and whole-process re-rating |
...
![Figure 20.5: Specified utilization scenario over a hypothetical field lifecycle](figures/utilization_trend_field_life.png)

<!-- scientific-illustration:utilization_trend_field_life.png -->
The plateau and decline are assigned teaching trajectories. They illustrate time-dependent capacity demand and are not forecasts from a calibrated reservoir or facility model.
<!-- /scientific-illustration -->
...
# Explicit pressure convention: 10% overpressure applied to gauge set pressure
P_design = 75.0  # bara
P_atm = 1.01325
...
# Gas-phase properties at the specified relief state; no device sizing certification
fluid = feed.getFluid()
...
# Independent SI ideal-gas sonic-nozzle illustration (NASA Glenn mass-flow relation).
...
![Figure 20.6: Equipment Capacity Utilization at Nominal Feed Rate](figures/ch18_utilization_bar_chart.png)
...
![Figure 20.7: Equipment Utilization vs Feed Rate](figures/ch18_utilization_vs_feed_rate.png)
...
6. **A proportional-load screening estimate** is $Q_{\text{screen}} = Q_{\text{current}} / U_{\text{bottleneck}}$ only when all relevant loads scale linearly at fixed ratings. A solved facility capacity requires rerunning the process and all applicable constraints at the candidate rate. Temperature-approach indicators are not throughput ratings.
...
For the process model in Exercise 20.7, fix the total mass flow at 350,000 kg/hr and vary the water mole fraction from 0.05 to 0.50 (adjusting the methane fraction to maintain total = 1.0). Report separator liquid utilization against the specified water mole fraction. Compute a separate water-cut volume at stated reference conditions before using that term, and report whether any tested case reaches the declared liquid-handling screen.
...
5. API RP 14E (1991). *Recommended Practice for Design and Installation of Offshore Production Platform Piping Systems*, 5th edn. Washington, DC: American Petroleum Institute.
...
10. NORSOK P-002 (2023, corrected 2024). *Process System Design*. Lysaker: Standards Norway.
...
12. Lieberman, N.P. (2009). *Troubleshooting Process Plant Control*. Wiley. DOI: 10.1002/9780470432259.
