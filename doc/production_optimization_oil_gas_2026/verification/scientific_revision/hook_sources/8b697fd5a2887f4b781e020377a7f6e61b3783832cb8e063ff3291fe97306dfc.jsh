...
BookSolutionChecks.destination=Paths.get("C:\\Users\\solbraa\\OneDrive - NTNU\\Documents\\GitHub\\neqsim\\neqsim-paperlab\\books\\production_optimization_oil_gas_2026\\.build\\java_solution_runs\\ch30_digital_twins_and_automation\\checks.json");
...
BookSolutionChecks.process(process); BookSolutionChecks.check("lifecycle serialized state schema valid",result.isValid(),0,0);
...
BookSolutionChecks.check("multi-area saved state exists",Files.size(Paths.get("platform_x_v3.json"))>100,0,0);
...
BookSolutionChecks.check("metadata-only version comparison preserves equipment",diff.getAddedEquipment().isEmpty()&&diff.getRemovedEquipment().isEmpty(),0,0);
BookSolutionChecks.fence=27;
...
BookSolutionChecks.check("compressed state roundtrip semantic JSON",new com.google.gson.JsonParser().parse(modelState.toJson()).equals(new com.google.gson.JsonParser().parse(restored.toJson())),0,0);
