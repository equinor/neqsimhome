...
def after_fence(n,i,ns,code):
    ...
    if n==30 and i==18:
        for name,v in ns['comparison'].items():
            expected=abs(v['model']-v['measured'])/v['measured']*100
    ...
    if n==30 and i==21:
        ...
        rows.append(check('agent optimizer finite bounded pressure settings',all(math.isfinite(float(v)) for v in settings.values())))
...
def network_checks(i,ns):
    ...
    if i==11:
        ...
        return [check('native lift allocation budget',q1+q2,2.5e6,1.),
                check('native lift objective direct replay',float(r.getTotalOil()),float(replay),1e-6),
                check('native lift objective matches dense grid',float(replay),float(grid),1.)]
    if i==12:
