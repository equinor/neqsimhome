...
from optimization_additional_solution_checks import *
TARGET_FENCES={23:[1,3,10,15,16,22,23,25,27,28,31,35,37,39],24:[5,13,16,17,18,23,33,34,41,44,59,62,69,72,73,74,75,80],
               25:[1,4,5,9,10,11,12,13,14,15,17],26:[2,4,5,6,8,11,12,13,14,15,16],29:[2,5,6,13],
               27:[1,2,3,6,7],30:[9,13,14,15,18,19,20,21,28,29],31:[2],
               32:[2,4,6,10,20,21,22,24,25],35:[6]}
...
def on_trace(n,i,frame,event,arg):
    ...
    if key in _pending and event in ('line','return'):
        ...
        if obj is not None:
            try:_trace_checks[n,i].append({'line':frame.f_lineno,'checks':process_checks(obj),'accepted':True})
            except AssertionError as exc:
                if (n,i)!=(24,73):raise
                _trace_checks[n,i].append({'line':frame.f_lineno,'accepted':False,'rejection_diagnostic':str(exc)})
    if event=='line' and ':fence' in frame.f_code.co_filename:
        defining_fence=int(frame.f_code.co_filename.rsplit(':fence',1)[1])
        lines=_lines[n,defining_fence]
...
def after_fence(n,i,ns,code):
    ...
    if n==27:return scenario_checks(i,ns)
    if n==32 and i!=2:return advanced_checks(i,ns)
    if n==35:return hydrogen_checks(ns)
    ...
    if n==24 and i in [16,18,69]:
        # Normalized Python map is a fraction; native Java map is already percent.
        if i==69:
            saved_rate=float(ns['feed'].getFlowRate('kg/hr'))
            ns['feed'].setFlowRate(250000.,'kg/hr');ns['run_facility']()
        for name,value in ns['summary'].items():
            rows.append(check(name+' native percent-to-fraction identity',value,
                float(ns['process'].getUnit(name).getMaxUtilization()),1e-12))
        if i==69:
            ns['feed'].setFlowRate(saved_rate,'kg/hr');ns['run_facility']()
    if n==25 and i in [4,9,15,17]:
        values=ns['snapshot']['utilization'] if i==9 else ns['record']['utilization'] if i==17 else ns['utilization']
        for name,item in values.items():
            value=item['value'] if i==17 else item
            expected=float(process.getUnit(name).getMaxUtilization())
            rows.append(check(name+' dashboard fraction versus native equipment readback',value,expected,.000500001 if i==17 else 1e-12))
        if i==9:
            toy=[{'timestamp':0.,'utilization':{'K':.6}}, {'timestamp':86400.,'utilization':{'K':.7}}]
            rows.append(check('independent utilization trend perday',ns['utilization_trend'](toy,'K'),.1,1e-12))
        if i==15:rows.append(check('status categories partition all records',ns['green']+ns['yellow']+ns['red'],len(values)))
    if n==29 and i==2:
        rows.extend(boundary('Entire steady recycle train',[ns['feed']],
                    [ns['splitter'].getSplitStream(0)],
                    float(ns['compressor'].getPower())+float(ns['aftercooler'].getDuty())))
    if _trace_checks.get((n,i)):
        trace_rows=_trace_checks[n,i]
        rows.append({'name':'Every observed literal process.run state passes unit boundaries or is explicitly rejected by the declared candidate gate',
                     'passed':True,'observed':len(trace_rows),'states':trace_rows})
    if n in (23,24):rows.extend(optimizer_api_checks(n,i,ns))
    if n==30 and i==9:
    ...
    if n==32 and i==2:
        ...
        for a,(q,w) in enumerate(zip(production,power)):
            ...
            rows.append(check('Pareto point '+str(a)+' independent grid power scaling',w/q,ns['grid_power'][0]/ns['grid_production'][0],1e-6))
        rows.append(check('independent25-point trade-off grid is strictly increasing in both objectives',
                          len(ns['grid_power'])==25 and all(b>a for a,b in zip(ns['grid_power'][:-1],ns['grid_power'][1:]))))
    if n==25 and i==10:
...
def network_checks(i,ns):
    ...
    if i in [8,13]:
        network=ns['network']
        q1=float(network.getPipeFlowRate('Flowline-A'));q2=float(network.getPipeFlowRate('Export'))
        rows=[check('network solver converged',bool(network.isConverged())),
                check('internal junction mass conservation kg/hr',q1,q2,1e-5),
                check('fixed pressure source boundary',float(network.getNodePressure('Supply-A')),90.,1e-9),
                check('fixed pressure sink boundary',float(network.getNodePressure('Platform')),85.,1e-9),
                check('no pump passive pressure ordering',90.>float(network.getNodePressure('Manifold'))>85.),
        ...
        for name in network.getPipeNames():
            pipe=network.getPipe(name)
            rows.append(check(name+' flow getter converts internal kg/s to kg/hr',float(network.getPipeFlowRate(name)),float(pipe.getFlowRate())*3600.,1e-6))
        return rows
    if i==14:
        r=ns['answer'];a=dict(r.getLiftRates());q1=float(a['Well-A']);q2=float(a['Well-B'])
        replay=float(ns['curve_a'].oilRateAt(q1)+ns['curve_b'].oilRateAt(q2))
        return [check('repeated native lift candidate budget',q1+q2<=2.5e6+1e-6),
                check('repeated native candidate objective replay',float(r.getTotalOil()),replay,1e-6),
                check('native candidate remains below accepted7050 LP optimum',replay<7050.-1)]
    if i==11:
