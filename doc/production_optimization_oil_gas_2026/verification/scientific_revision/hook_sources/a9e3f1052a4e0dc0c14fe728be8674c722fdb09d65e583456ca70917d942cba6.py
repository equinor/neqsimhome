...
def optimizer_api_checks(n,i,ns):
 ...
 if (n,i) in [(23,28),(24,34)]:
  ...
  for p in table.getPoints():
   rate=float(p.getMaxFlowRate());rows.append(check('capacity table finite in declared rate bounds',math.isfinite(rate) and 0<=rate<=(200000. if n==23 else 300000.)+1e-6))
