...
def after_fence(ch,i,ns,code):
    ...
    if i==13:
        ...
        if i==22:assert np.allclose(ns['bhp_data'],expected,equal_nan=True)
            ...
            for gi,line in enumerate(ns['ax'].lines):
                assert np.array_equal(np.asarray(line.get_xdata()),np.asarray(table.getFlowRates())/1000.)
                assert np.array_equal(np.asarray(line.get_ydata()),[table.getBHP(a,1,0,gi) for a in range(3)])
