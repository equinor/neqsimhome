...
def screen(p):
    ...
    for name in ['HP Sep','Export Comp']:
        ...
        else:raise AssertionError('Accepted capacity screen requires an enabled constraint for '+name)
    return max(values)
...
def after_fence(n,i,ns,code):
    ...
    if i==25:
        ...
        plot_data['waterfall_debottleneck']={'base_rate_kg_hr':ns['base_rate'],'screened_rate_kg_hr':ns['current_rate'],'steps':ns['bottlenecks']}
    if i in (29,31,32,39):
        vals={str(e.getKey()):float(e.getValue()) for e in ns['process'].getCapacityUtilizationSummary().entrySet()}
        check(i,'utilization table percent basis',vals and all(math.isfinite(v) and v>=0 and abs(v-100*ns['process'].getUnit(k).getMaxUtilization())<1e-8 for k,v in vals.items()),vals,'summary is percent, exactly100 times dimensionless equipment utilization; >100 means overload')
        if i==31:
            check(i,'dashboard percent readback',all(abs(v-vals[k])<1e-8 for k,v in zip(ns['names'],ns['values'])),ns['values'],'plotted percentages match API percentage values without multiplying again')
            plot_data['utilization_dashboard']={'equipment':ns['names'],'utilization_percent':ns['values']}
    ...
    if i==32:
        ...
        plot_data['bottleneck_sensitivity']={'feed_rate_kg_hr':list(ns['flow_rates']),'equipment':ns['bottleneck_names'],'utilization_percent':ns['bottleneck_utils']}
        check(i,'outside-map penalty separated from numerical curve',all((math.isnan(v) if invalid else abs(v-raw)<1e-10) for raw,v,invalid in zip(ns['bottleneck_utils'],ns['plotted_utilization'],ns['outside_map'])),{'rejected_map_cases':int(np.count_nonzero(ns['outside_map']))},'999percent penalty is displayed as a rejected map state, not physical utilization')
        plot_data['bottleneck_sensitivity']['outside_map_flags']=[bool(v) for v in ns['outside_map']]
    if i==36:
        ...
        for row in ns['economic_rows']:
            annual=row['delta_q_bpd']*365*ns['uptime']*ns['oil_price_usd_bbl']*ns['nok_per_usd']/1e6
            expected=annual*factor-row['capex_mnok']
            breakeven=row['capex_mnok']*1e6/(row['delta_q_bpd']*365*ns['uptime']*ns['nok_per_usd']*factor)
            check(i,row['name']+' cashflow units and break-even',abs(row['npv_mnok']-expected)<1e-8 and abs(row['PI']-ns['oil_price_usd_bbl']/breakeven)<1e-10,row,'bbl/day×days/year×USD/bbl×NOK/USD /1e6; closed annuity; PI=price/break-even price')
    if i==39:
        ...
        check(i,'report percentages agree with API',all(abs(v-round(vals[k],1))<1e-8 for k,v in r['equipment_utilization'].items()),r['equipment_utilization'],'one-decimal API percentages')
        path=Path(__file__).resolve().parents[1]/'verification/scientific_revision/ch21_plot_data.json'
        path.write_text(json.dumps(plot_data,indent=2,default=float),encoding='utf-8')
    return checks[i]
