...
def advanced_checks(i,ns):
 ...
 if i==4:
  ...
  selected=ns['selected'];c,physical=simple_compression(ns,float(selected[0])*100000.,150.,float(selected[1])*50.,313.15)
  rows.extend(physical)
  rows.append(check('retained SQP/baseline point independently replayed',float(ns['final_power']),float(c.getPower('kW')),max(1e-4,float(ns['final_power'])*1e-6)))
  rows.append(check('retained point independently meets5000kW limit',float(c.getPower('kW'))<=5001.))
  return rows
 if i in (6,10):
 ...
 if i==20:
  ...
  for q,p,w in zip(ns['flows'],ns['pressures'],ns['powers']):
   ...
   actual_feed=ns['process'].getUnit('Feed');actual_comp=ns['process'].getUnit('Compressor')
   c,physical=simple_compression(ns,q,p,inlet_pressure=float(actual_feed.getPressure('bara')),
                                  temperature=float(actual_feed.getTemperature('K')),
                                  fluid=actual_feed.getFluid())
   c.setPolytropicEfficiency(float(actual_comp.getPolytropicEfficiency()));c.run()
...
def optimizer_api_checks(n,i,ns):
 ...
 if (n,i) in [(23,28),(24,34)]:
  ...
  for p in table.getPoints():
   ...
   if math.isnan(rate):
    ...
    rows.append(check('native missing capacity rejected; independent feasible bounded grid supplied',50000.<=grid['grid_maximum_kg_hr']<=(200000. if n==23 else 300000.)))
    continue
 ...
 if n==24 and i==62:
  rev=[q/0.158987294928*70/1000+g*1e6*38/1055.05585262*8/1000-w*24*.05 for q,g,w in zip(ns['oil_rates'],ns['gas_rates'],ns['comp_powers'])]
  ...
  rows.append(check('explicit fixed feed rate removes prior optimizer state dependence',float(ns['feed'].getFlowRate('kg/hr')),250000.,1e-6))
  rows.append(check('all28 flash pressure cases retained',len(ns['stock_tank_results']),28))
  for index,tank in enumerate(ns['stock_tank_results']):
   prefix='stock-tank pressure point'+str(index)+' '
   rows.extend([check(prefix+'reference temperature',tank['temperature_K'],288.15,1e-10),
                check(prefix+'reference absolute pressure',tank['pressure_bara'],1.01325,1e-10),
                check(prefix+'all phase mass retained',tank['mass_relative_error'],0.,1e-6),
                check(prefix+'all phase components retained',tank['component_relative_error'],0.,1e-6),
                check(prefix+'isothermal conditioning heat identity',tank['energy_relative_error'],0.,1e-5)])
   oil=next(r for r in tank['products'] if r['phase']=='oil')
   rows.append(check(prefix+'condensate density domain',450.<oil['density_kg_m3']<1000.))
   rows.append(check(prefix+'m3/day from oil mass and density, not ideal-gas moles',ns['oil_rates'][index],24.*oil['mass_kg_hr']/oil['density_kg_m3'],1e-7))
   rows.append(check(prefix+'flash oil mass cannot exceed separated feed mass',oil['mass_kg_hr']<=tank['feed_mass_kg_hr']*(1.+1e-6)))
   for phase in tank['products']:
    rows.append(check(prefix+phase['phase']+' nonnegative actual phase volume',phase['actual_m3_hr']>=0.))
   if any(r['phase']=='gas' for r in tank['products']):
    gas=next(r for r in tank['products'] if r['phase']=='gas')
    rows.append(check(prefix+'reference oil denser than flash gas',oil['density_kg_m3']>100.*gas['density_kg_m3']))
 if n==24 and i==73:
  all_rows=ns['results_matrix'];passed=[r for r in all_rows if r['accepted']];failed=[r for r in all_rows if not r['accepted']]
  rows.append(check('32 attempts preserve30 accepted and2 rejected native states',(len(all_rows),len(passed),len(failed)),(32,30,2)))
  rows.append(check('no rejected state eligible for preferred pressure choice',all(r['accepted'] for r in ns['best_by_rate'].values())))
  for rate,best in ns['best_by_rate'].items():
   rows.append(check('minimum-power selection over accepted grid atfixedrate'+str(rate),best,min([r for r in passed if r['rate_kg_hr']==rate],key=lambda r:r['power_MW'])))
 if n==24 and i==75:
